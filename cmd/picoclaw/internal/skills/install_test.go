...
func TestNewInstallSubcommand(t *testing.T) {
	...
	assert.Equal(t, "Установить навык из GitHub", cmd.Short)
...
func TestInstallCommandArgs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			args:        []string{"Perdonus/ruclaw-skills/weather"},
			registry:    "",
			expectError: false,
		},
		{
			...
			registry:    "",
			expectError: true,
			errorMsg:    "требуется ровно 1 аргумент: <github>",
		},
		{
		...
		},
		{
			...
			registry:    "clawhub",
			expectError: true,
			errorMsg:    "если указан --registry, требуется ровно 1 аргумент: <slug>",
		},
		{
