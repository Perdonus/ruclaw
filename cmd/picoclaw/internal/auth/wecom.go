...
func newWeComCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "wecom",
		Short: "Отсканировать QR-код WeCom и подключить канал WeCom к RuClaw",
		Args:  cobra.NoArgs,
	...
	cmd.Flags().DurationVar(&timeout, "timeout", wecomQRPollTimeout, "Сколько ждать подтверждения QR-кода")
...
func authWeComCmdWithScanner(
	...
	if scanner == nil {
		return fmt.Errorf("сканер QR-кода WeCom не задан")
	}
	if writer == nil {
		writer = os.Stdout
	}
	...
	if saveErr := config.SaveConfig(internal.GetConfigPath(), cfg); saveErr != nil {
		return fmt.Errorf("не удалось сохранить конфигурацию: %w", saveErr)
	}

	fmt.Fprintln(writer)
	fmt.Fprintln(writer, "WeCom подключён к RuClaw.")
	fmt.Fprintf(writer, "ID бота: %s\n", botInfo.BotID)
	fmt.Fprintf(writer, "Файл конфигурации: %s\n", internal.GetConfigPath())
...
func scanWeComQRCodeInteractive(ctx context.Context, opts wecomQRFlowOptions) (wecomQRBotInfo, error) {
	...
	fmt.Fprintln(opts.Writer, "Запрашиваю QR-код WeCom...")
	...
	fmt.Fprintln(opts.Writer, "=======================================================")
	fmt.Fprintln(opts.Writer, "Отсканируйте QR-код ниже через WeCom:")
	fmt.Fprintln(opts.Writer, "=======================================================")
	fmt.Fprintln(opts.Writer)
	...
	fmt.Fprintln(opts.Writer)
	fmt.Fprintf(opts.Writer, "Ссылка на QR-код: %s\n", pageURL)
	fmt.Fprintln(opts.Writer)
	fmt.Fprintln(opts.Writer, "Ожидание сканирования...")
...
func fetchWeComQRCode(ctx context.Context, opts wecomQRFlowOptions) (wecomQRSession, error) {
	...
	if err := doWeComJSONGet(ctx, opts.HTTPClient, generateURL, &resp); err != nil {
		return wecomQRSession{}, fmt.Errorf("не удалось получить QR-код WeCom: %w", err)
	}
	if resp.ErrCode != 0 {
		return wecomQRSession{}, fmt.Errorf(
			"не удалось получить QR-код WeCom: errcode=%d errmsg=%s",
			resp.ErrCode,
			resp.ErrMsg,
		)
	}
	if resp.Data.SCode == "" || resp.Data.AuthURL == "" {
		return wecomQRSession{}, fmt.Errorf("не удалось получить QR-код WeCom: в ответе нет scode или auth_url")
	}
...
func pollWeComQRCodeResult(ctx context.Context, opts wecomQRFlowOptions, scode string) (wecomQRBotInfo, error) {
	if strings.TrimSpace(scode) == "" {
		return wecomQRBotInfo{}, fmt.Errorf("отсутствует scode для QR-кода WeCom")
	}
	...
	for {
		...
		if err != nil {
			if errors.Is(err, context.DeadlineExceeded) || errors.Is(timeoutCtx.Err(), context.DeadlineExceeded) {
				return wecomQRBotInfo{}, fmt.Errorf("истекло время ожидания сканирования QR-кода WeCom (%s)", opts.PollTimeout)
			}
			return wecomQRBotInfo{}, err
		}
		...
		case "success":
			if status.Data.BotInfo.BotID == "" || status.Data.BotInfo.Secret == "" {
				return wecomQRBotInfo{}, fmt.Errorf("QR-код WeCom подтверждён, но учётные данные бота отсутствуют")
			}
		...
		case "expired":
			return wecomQRBotInfo{}, fmt.Errorf("QR-код WeCom истёк, попробуйте снова")
		case "scaned", "scanned":
			if !scannedPrinted {
				fmt.Fprintln(opts.Writer, "QR-код отсканирован. Подтвердите вход в WeCom.")
				scannedPrinted = true
			}
		}
...
func queryWeComQRCodeStatus(ctx context.Context, opts wecomQRFlowOptions, scode string) (wecomQRQueryResponse, error) {
	...
	if err := doWeComJSONGet(ctx, opts.HTTPClient, queryURL, &resp); err != nil {
		return wecomQRQueryResponse{}, fmt.Errorf("не удалось получить результат QR-кода WeCom: %w", err)
	}
	if resp.ErrCode != 0 {
		return wecomQRQueryResponse{}, fmt.Errorf(
			"не удалось получить результат QR-кода WeCom: errcode=%d errmsg=%s",
			resp.ErrCode,
			resp.ErrMsg,
		)
	}
...
func buildWeComQRGenerateURL(baseURL, sourceID string, platformCode int) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("некорректный URL генерации QR-кода WeCom: %w", err)
	}
...
func buildWeComQRQueryURL(baseURL, scode string) (string, error) {
	u, err := url.Parse(baseURL)
	if err != nil {
		return "", fmt.Errorf("некорректный URL запроса QR-кода WeCom: %w", err)
	}
...
func buildWeComQRCodePageURL(baseURL, sourceID, scode string) (string, error) {
	u, err := url.Parse(baseURL)
	if err != nil {
		return "", fmt.Errorf("некорректный URL страницы QR-кода WeCom: %w", err)
	}
...
func doWeComJSONGet(ctx context.Context, client *http.Client, targetURL string, out any) error {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return fmt.Errorf("неожиданный статус %s: %s", resp.Status, strings.TrimSpace(string(body)))
	}

	if err := json.NewDecoder(resp.Body).Decode(out); err != nil {
		return fmt.Errorf("декодирование JSON-ответа: %w", err)
	}
