...
func TestNewModelCommand(t *testing.T) {
	...
	assert.Equal(t, "Показать или изменить модель по умолчанию", cmd.Short)
...
func TestShowCurrentModel_WithDefaultModel(t *testing.T) {
	...
	assert.Contains(t, output, "Текущая модель по умолчанию: gpt-4")
	assert.Contains(t, output, "Доступные модели в config:")
...
func TestShowCurrentModel_NoDefaultModel(t *testing.T) {
	...
	assert.Contains(t, output, "Модель по умолчанию сейчас не задана.")
	assert.Contains(t, output, "Доступные модели в config:")
}
...
func TestListAvailableModels_Empty(t *testing.T) {
	...
	assert.Contains(t, output, "В model_list не настроено ни одной модели")
}
...
func TestSetDefaultModel_SaveConfigError(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "не удалось сохранить config")
}
...
func TestModelCommandExecution_Show(t *testing.T) {
	...
	assert.Contains(t, output, "Текущая модель по умолчанию: test-model")
}
