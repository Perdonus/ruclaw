...
func EvalSeahorse(
	...
	for si := range samples {
		...
		if !ok {
			log.Printf("ПРЕДУПРЕЖДЕНИЕ: нет conversation ID для сэмпла %s", sample.SampleID)
			continue
		}
		...
		for qi := range sample.QA {
			...
			for _, kw := range keywords {
				...
				if err != nil {
					log.Printf("ПРЕДУПРЕЖДЕНИЕ: поиск по ключевому слову %q не удался: %v", kw, err)
					continue
				}
			...
			if len(messageIDs) > 0 {
				...
				if err != nil {
					log.Printf("ПРЕДУПРЕЖДЕНИЕ: expand для сэмпла %s не удался: %v", sample.SampleID, err)
				} else {
...
func SaveResults(results []EvalResult, outDir string) error {
	...
	for _, r := range results {
		...
		if err != nil {
			return fmt.Errorf("сериализовать результат: %w", err)
		}
...
func printSection(title string, results []EvalResult) {
	...
	fmt.Printf("%-10s %-8s %-8s", "Режим", "HitRate", "F1")
...
func PrintComparison(results []EvalResult, llmResults []EvalResult) {
	printSection("Без генерации LLM", results)
	if len(llmResults) > 0 {
		printSection("С LLM", llmResults)
	}
}
