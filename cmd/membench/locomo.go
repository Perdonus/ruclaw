...
func LoadDataset(dataDir string) ([]LocomoSample, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("прочитать каталог данных %s: %w", dataDir, err)
	}
	...
	for _, entry := range entries {
		if !entry.IsDir() && strings.HasSuffix(entry.Name(), ".json") {
			...
			if err != nil {
				return nil, fmt.Errorf("прочитать файл %s: %w", path, err)
			}
			var batch []LocomoSample
			if err := json.Unmarshal(data, &batch); err != nil {
				return nil, fmt.Errorf("разобрать файл %s: %w", path, err)
			}
...
func GetTurns(sample *LocomoSample) []LocomoTurn {
	...
	for _, name := range names {
		...
		if err := json.Unmarshal(raw, &turns); err != nil {
			log.Printf("ПРЕДУПРЕЖДЕНИЕ: unmarshal для session %q в сэмпле %s не удался: %v", name, sample.SampleID, err)
			continue
		}
