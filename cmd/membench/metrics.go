...
func RecallHitRate(evidenceIDs []string, sample *LocomoSample, retrievedContent string) float64 {
	...
	if len(expanded) == 0 {
		log.Printf("ПРЕДУПРЕЖДЕНИЕ: после разворачивания evidence %v не осталось корректных dia_id", evidenceIDs)
	...
	for _, diaID := range expanded {
		...
		if !ok {
			log.Printf("ПРЕДУПРЕЖДЕНИЕ: dia_id %q не найден в сэмпле %s", diaID, sample.SampleID)
			continue
		}
		resolvable++
		if len(turn.Text) < 20 {
			log.Printf("ПРЕДУПРЕЖДЕНИЕ: короткий текст хода (%d символов) для dia_id %s: %q",
