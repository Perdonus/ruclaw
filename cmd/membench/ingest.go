...
import (
	...
	"github.com/Perdonus/ruclaw/pkg/seahorse"
)
...
func IngestSeahorse(ctx context.Context, samples []LocomoSample, dbPath string) (*SeahorseIngestResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("создать движок seahorse: %w", err)
	}
	...
	for si := range samples {
		...
		if existing != nil {
			...
			log.Printf("Пропускаю существующий сэмпл %s: convID=%d", sample.SampleID, existing.ConversationID)
			continue
		}
		...
		if err != nil {
			return nil, fmt.Errorf("загрузить сэмпл %s: %w", sample.SampleID, err)
		}

		// Get the conversation ID for scoped retrieval
		conv, err := store.GetConversationBySessionKey(ctx, sessionKey)
		if err != nil {
			return nil, fmt.Errorf("получить conversation для %s: %w", sample.SampleID, err)
		}
		if conv == nil {
			return nil, fmt.Errorf("conversation для %s не найдена после загрузки", sample.SampleID)
		}
		convMap[sample.SampleID] = conv.ConversationID
		log.Printf("Загружен сэмпл %s: %d ходов, convID=%d", sample.SampleID, len(turns), conv.ConversationID)
	}

	log.Printf("Загрузка Seahorse завершена: %d сэмплов, %d conversations", len(samples), len(convMap))
