...
func displaySchemeType(value string) string {
	switch value {
	case "openai-compatible":
		return "OpenAI-совместимый"
	case "anthropic":
		return "Anthropic"
	default:
		return value
...
func (a *App) newSchemesPage() tview.Primitive {
	...
	table.SetBorder(true).
		SetTitle(" [#00f0ff::b] СХЕМЫ ПРОВАЙДЕРОВ ").
	...
	rebuild := func() {
		...
		for i, s := range schemes {
			...
			table.SetCell(detailRow, 0,
				tview.NewTableCell(fmt.Sprintf("  [#808080]доступно: %d/%d · %s", m, n, s.BaseURL)).
					SetTextColor(tcell.NewHexColor(0x808080)).
					SetExpansion(1).
					SetSelectable(false),
			)
			table.SetCell(detailRow, 1,
				tview.NewTableCell("[#00f0ff]"+displaySchemeType(s.Type)+"  ").
	...
	table.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		...
		case 'd':
			...
			name := schemes[idx].Name
			a.confirmDelete(fmt.Sprintf("схему %q", name), func() {
				current := a.cfg.Provider.Schemes
	...
	return a.buildShell(
		"schemes",
		table,
		" [#00f0ff]a:[-] добавить  [#00f0ff]e:[-] изменить  [#ff2a2a]d:[-] удалить  [#39ff14]Enter:[-] открыть  [#ff00ff]ESC:[-] назад ",
	)
}
...
func (a *App) showSchemeForm(existing *tuicfg.Scheme, onSave func(tuicfg.Scheme)) {
	...
	typeValues := []string{"openai-compatible", "anthropic"}
	typeLabels := []string{"OpenAI-совместимый", "Anthropic"}
	typeIdx := 0
	...
	form.
		...
		AddInputField("Базовый URL", baseURL, 28, nil, func(text string) { baseURL = text }).
		AddDropDown("Тип", typeLabels, typeIdx, func(_ string, index int) {
			if index >= 0 && index < len(typeValues) {
				schemeType = typeValues[index]
			}
		}).
		AddButton("СОХРАНИТЬ", func() {
			...
			if baseURL == "" {
				a.showError("Базовый URL обязателен")
				return
			}
			if existing == nil {
				for _, s := range a.cfg.Provider.Schemes {
					if s.Name == name {
						a.showError(fmt.Sprintf("Схема %q уже существует", name))
						return
					}
				}
			}
