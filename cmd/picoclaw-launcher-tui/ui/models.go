...
func (a *App) newModelsPage(schemeName, userName, baseURL string) tview.Primitive {
	...
	table.SetBorder(true).
		SetTitle(fmt.Sprintf(" [#00f0ff::b] МОДЕЛИ · %s / %s ", schemeName, userName)).
	...
	status := tview.NewTextView().
		...
		SetText("[#ffff00]ЗАГРУЗКА МОДЕЛЕЙ...[-]")
	...
	go func() {
		...
		var err error
		if apiKey == "" {
			err = fmt.Errorf("ключ обязателен")
		} else {
		...
		a.tapp.QueueUpdateDraw(func() {
			if err != nil {
				status.SetText(fmt.Sprintf("[#ff2a2a]ОШИБКА: %s[-]", err.Error()))
				table.SetCell(0, 0, tview.NewTableCell(" (не удалось загрузить модели)"))
				a.tapp.SetFocus(table)
				return
			}
			if len(entries) == 0 {
				status.SetText("[#ff2a2a]МОДЕЛИ НЕ НАЙДЕНЫ[-]")
				table.SetCell(0, 0, tview.NewTableCell(" (нет доступных моделей)"))
				a.tapp.SetFocus(table)
				return
			}

			status.SetText(fmt.Sprintf("[#39ff14]Загружено моделей: %d[-]", len(entries)))
	...
	return a.buildShell("models", flex, " [#39ff14]Enter:[-] выбрать  [#ff00ff]ESC:[-] назад ")
}
...
func fetchModels(baseURL, apiKey string) ([]modelEntry, error) {
	...
	return nil, fmt.Errorf(
		"не удалось разобрать ответ: неизвестный формат: %s",
