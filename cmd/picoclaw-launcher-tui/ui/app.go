...
func (a *App) save() {
	if err := tuicfg.Save(a.configPath, a.cfg); err != nil {
		a.showError("Не удалось сохранить: " + err.Error())
	}
}

func (a *App) showError(msg string) {
	modal := tview.NewModal().
		SetText(" [red::b]ОШИБКА[-::-]\n\n" + msg).
...
func (a *App) confirmDelete(label string, onConfirm func()) {
	modal := tview.NewModal().
		SetText(" [red::b]ПОДТВЕРЖДЕНИЕ УДАЛЕНИЯ[-::-]\n\nУдалить " + label + "?\n[gray]Это действие нельзя отменить.[-]").
		AddButtons([]string{"Удалить", "Отмена"}).
		SetDoneFunc(func(_ int, buttonLabel string) {
			a.hideModal("confirm-delete")
			if buttonLabel == "Удалить" {
				onConfirm()
			}
		})
	// Cyberpunk Modal Style
...
func (a *App) buildShell(pageID string, content tview.Primitive, hint string) tview.Primitive {
	...
	if pageID == "home" {
		...
		modelTV.SetText("МОДЕЛЬ: " + a.cfg.CurrentModelLabel() + " ")
	} else {
	...
	headerLeft := tview.NewTextView().
		SetText(" [#ff00ff::b]///[#00f0ff] RuClaw ЛАУНЧЕР [#ff00ff]///").
		SetDynamicColors(true).
		SetBackgroundColor(tcell.NewHexColor(0x050510))
	...
	sbText += menuItem("home", "ГЛАВНАЯ")
	sbText += menuItem("schemes", "СХЕМЫ")
	sbText += menuItem("users", "ПОЛЬЗОВ.")
	sbText += menuItem("models", "МОДЕЛИ")
	sbText += menuItem("channels", "КАНАЛЫ")
	sbText += menuItem("gateway", "ШЛЮЗ")
