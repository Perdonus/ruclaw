...
func startGateway() error {
	...
	if status.running {
		return fmt.Errorf("шлюз уже запущен (PID: %d)", status.pid)
	}
	...
	if runtime.GOOS == "windows" {
		...
		if err != nil {
			return fmt.Errorf("не удалось получить PID шлюза: %w", err)
		}
	...
	status = getGatewayStatus()
	if !status.running {
		return fmt.Errorf("не удалось запустить шлюз")
	}
	return nil
}
...
func stopGateway() error {
	status := getGatewayStatus()
	if !status.running {
		return fmt.Errorf("шлюз не запущен")
	}
...
func (a *App) newGatewayPage() tview.Primitive {
	...
	flex.SetBorder(true).
		SetTitle(" [#00f0ff::b] УПРАВЛЕНИЕ ШЛЮЗОМ ").
	...
	buttons.AddItem(" [lime]ЗАПУСК[white]  ", "", 0, func() {
		if !getGatewayStatus().running {
			err := startGateway()
			if err != nil {
	...
	buttons.AddItem(" [red]СТОП[white]    ", "", 0, func() {
		if getGatewayStatus().running {
			err := stopGateway()
			if err != nil {
				a.showError(err.Error())
	...
	updateStatus = func() {
		...
		if status.running {
			versionInfo := ""
			if status.version != "" {
				versionInfo = fmt.Sprintf("\nВерсия: %s", status.version)
			}
			statusTV.SetText(fmt.Sprintf("[#39ff14::b]ШЛЮЗ ЗАПУЩЕН[-]\n\nPID: %d%s", status.pid, versionInfo))
			buttons.SetItemText(0, " [gray]ЗАПУСК[white]  ", "")
			buttons.SetItemText(1, " [red]СТОП[white]    ", "")
		} else {
			statusTV.SetText("[#ff2a2a::b]ШЛЮЗ ОСТАНОВЛЕН[-]\n\nPID: Н/Д")
			buttons.SetItemText(0, " [lime]ЗАПУСК[white]  ", "")
			buttons.SetItemText(1, " [gray]СТОП[white]    ", "")
		}
	}
	...
	return a.buildShell("gateway", flex, " [#39ff14]Enter:[-] выбрать  [#ff2a2a]ESC:[-] назад ")
}
