...
func (a *App) newChannelsPage() tview.Primitive {
	list := tview.NewList()
	list.SetBorder(true).
		SetTitle(" [#00f0ff::b] КАНАЛЫ СВЯЗИ ").
	...
	rebuild := func() {
		...
		if chRaw, ok := cfg["channels"].(map[string]any); ok {
			for name, ch := range chRaw {
				...
				list.AddItem(name, fmt.Sprintf("Статус: %s", enabled), 0, func() {
	...
	return a.buildShell("channels", list, " [#ff00ff]Enter:[-] изменить  [#ff2a2a]ESC:[-] назад ")
}

func (a *App) showChannelEditForm(configPath, channelName string, existing map[string]any) {
	form := tview.NewForm()
	form.SetBorder(true).
		SetTitle(" [::b]ИЗМЕНИТЬ КАНАЛ ").
	...
	form.AddButton("СОХРАНИТЬ", func() {
		...
		if channelName == "" {
			if nameField == nil || nameField.GetText() == "" {
				a.showError("Название канала обязательно")
				return
			}
		...
		if err != nil {
			a.showError(fmt.Sprintf("Не удалось сохранить конфиг: %v", err))
			return
		}
		if err := os.MkdirAll(filepath.Dir(configPath), 0o700); err != nil {
			a.showError(fmt.Sprintf("Не удалось создать каталог конфигурации: %v", err))
			return
		}
		if err := os.WriteFile(configPath, data, 0o600); err != nil {
			a.showError(fmt.Sprintf("Не удалось записать конфиг: %v", err))
			return
		}
