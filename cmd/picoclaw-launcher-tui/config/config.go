...
import (
	...
	"github.com/Perdonus/ruclaw/pkg/fileutil"
)
...
func Load(path string) (*TUIConfig, error) {
	...
	if _, err := toml.Decode(string(data), cfg); err != nil {
		return nil, fmt.Errorf("не удалось разобрать файл конфигурации %q: %w", path, err)
	}
...
func Save(path string, cfg *TUIConfig) error {
	...
	if err := fileutil.WriteFileAtomic(path, buf.Bytes(), 0o600); err != nil {
		return fmt.Errorf("не удалось записать файл конфигурации %q: %w", path, err)
	}
	return nil
}
