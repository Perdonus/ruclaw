...
LAUNCHER_BINARY_NAME=ruclaw-launcher
LEGACY_LAUNCHER_BINARY_NAME=picoclaw-launcher
LAUNCHER_TUI_BINARY_NAME=ruclaw-launcher-tui
LEGACY_LAUNCHER_TUI_BINARY_NAME=picoclaw-launcher-tui
BUILD_DIR=build
...
## build: Build the ruclaw binary for current platform
build: generate
	...
	@cd $(BUILD_DIR) && $(LNCMD) $(BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT) $(BINARY_NAME)$(EXT)
	@cd $(BUILD_DIR) && $(LNCMD) $(BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT) $(LEGACY_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT)
	@cd $(BUILD_DIR) && $(LNCMD) $(BINARY_NAME)$(EXT) $(LEGACY_BINARY_NAME)$(EXT)

## build-launcher: Build the ruclaw-launcher (web console) binary
build-launcher:
	@echo "Building $(LAUNCHER_BINARY_NAME) for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@GOARCH=${ARCH} $(MAKE) -C web build \
		OUTPUT="$(CURDIR)/$(BUILD_DIR)/$(LAUNCHER_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT)" \
		WEB_GO='$(WEB_GO)' \
		GO_BUILD_TAGS='$(GO_BUILD_TAGS)' \
		LDFLAGS='$(LDFLAGS)'
	@cd $(BUILD_DIR) && $(LNCMD) $(LAUNCHER_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT) $(LAUNCHER_BINARY_NAME)$(EXT)
	@cd $(BUILD_DIR) && $(LNCMD) $(LAUNCHER_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT) $(LEGACY_LAUNCHER_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT)
	@cd $(BUILD_DIR) && $(LNCMD) $(LAUNCHER_BINARY_NAME)$(EXT) $(LEGACY_LAUNCHER_BINARY_NAME)$(EXT)
	@echo "Build complete: $(BUILD_DIR)/$(LAUNCHER_BINARY_NAME)$(EXT)"

build-launcher-frontend:
	@$(MAKE) -C web build-frontend

## build-launcher-tui: Build the ruclaw-launcher TUI binary
build-launcher-tui:
	@echo "Building $(LAUNCHER_TUI_BINARY_NAME) for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@$(GO) build $(GOFLAGS) -o $(BUILD_DIR)/$(LAUNCHER_TUI_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT) ./cmd/picoclaw-launcher-tui
	@cd $(BUILD_DIR) && $(LNCMD) $(LAUNCHER_TUI_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT) $(LAUNCHER_TUI_BINARY_NAME)$(EXT)
	@cd $(BUILD_DIR) && $(LNCMD) $(LAUNCHER_TUI_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT) $(LEGACY_LAUNCHER_TUI_BINARY_NAME)-$(PLATFORM)-$(ARCH)$(EXT)
	@cd $(BUILD_DIR) && $(LNCMD) $(LAUNCHER_TUI_BINARY_NAME)$(EXT) $(LEGACY_LAUNCHER_TUI_BINARY_NAME)$(EXT)
	@echo "Build complete: $(BUILD_DIR)/$(LAUNCHER_TUI_BINARY_NAME)$(EXT)"
...
## build-android-arm64: Android app packaging is not available in this repo
build-android-arm64:
	@echo "Android packaging is not available in this repository."
	@echo "Missing Gradle/AndroidManifest/APK project sources; only desktop/CLI sources are present."
	@exit 1
...
## build-all: Build ruclaw for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
...
## install: Install ruclaw to system and copy builtin skills
install: build
	...
	@cd $(INSTALL_BIN_DIR) && $(LNCMD) $(BINARY_NAME)$(EXT) $(LEGACY_BINARY_NAME)$(EXT)
	@echo "Installed binary to $(INSTALL_BIN_DIR)/$(BINARY_NAME)"
	@echo "Installation complete!"

## uninstall: Remove ruclaw from system
uninstall:
...
## uninstall-all: Remove ruclaw and all data
uninstall-all:
...
vet: generate
	@packages="$$($(GO) list $(GOFLAGS) ./...)" && \
		$(GO) vet $(GOFLAGS) $$(printf '%s\n' "$$packages" | grep -v '^github.com/Perdonus/ruclaw/web/')
	@cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
	@$(GO) test $(GOFLAGS) $$($(GO) list $(GOFLAGS) ./... | grep -v github.com/Perdonus/ruclaw/web/)
	@cd web && make test
...
## docker-run: Run ruclaw gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run ruclaw gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run ruclaw agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm picoclaw-agent

## docker-run-agent-full: Run ruclaw agent in Docker (interactive, full-featured)
...
docker-clean:
	...
	docker rmi ruclaw:latest ruclaw:full 2>/dev/null || true


## build-macos-app: Build RuClaw macOS .app bundle (no terminal window)
build-macos-app:build-launcher
	...
	@echo "macOS .app bundle created: $(BUILD_DIR)/RuClaw Launcher.app"
