...
func TestShowListHandlers_ChannelPolicy(t *testing.T) {
	...
	if telegramReply != "Текущий канал: telegram" {
		t.Fatalf("telegram /show reply=%q, want=%q", telegramReply, "Текущий канал: telegram")
	}
	...
	if whatsappReply != "Текущий канал: whatsapp" {
		t.Fatalf("whatsapp /show reply=%q, want=%q", whatsappReply, "Текущий канал: whatsapp")
	}
