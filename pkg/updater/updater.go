...
func DownloadAndExtractRelease(releaseURL, platform, arch string) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		os.Remove(tmpPath)
		return "", fmt.Errorf("не удалось скачать артефакт: статус %d", resp.StatusCode)
	}
	...
	if checksum != "" {
		...
		if !strings.EqualFold(got, checksum) {
			_ = os.Remove(tmpPath)
			return "", fmt.Errorf("несовпадение checksum: получено %s, ожидалось %s", got, checksum)
		}
	}

	// Extract
	destDir, err := os.MkdirTemp("", "ruclaw-extract-*")
	if err != nil {
		os.Remove(tmpPath)
		return "", err
	}
...
func UpdateSelfFromRelease(releaseURL, platform, arch, programName string) error {
	...
	if err := selfupdate.Apply(f, opts); err != nil {
		return fmt.Errorf("не удалось применить обновление: %w", err)
	}
...
// Example: owner="Perdonus" -> https://api.github.com/repos/Perdonus/ruclaw/releases/latest
func GetReleaseAPIURL(owner string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/ruclaw/releases/latest", owner)
}

// GetProdReleaseAPIURL returns the default release API URL for the RuClaw fork.
func GetProdReleaseAPIURL() string {
	return GetReleaseAPIURL("Perdonus")
}

// GetReleaseTagAPIURL returns the GitHub Releases API URL for a specific tag.
// Example: owner="Perdonus", tag="nightly" -> https://api.github.com/repos/Perdonus/ruclaw/releases/tags/nightly
func GetReleaseTagAPIURL(owner, tag string) string {
	return fmt.Sprintf("https://api.github.com/repos/%s/ruclaw/releases/tags/%s", owner, tag)
}

// GetNightlyReleaseAPIURL returns the nightly release API URL for the RuClaw fork.
func GetNightlyReleaseAPIURL() string {
	return GetReleaseTagAPIURL("Perdonus", "nightly")
}
...
func findAssetInfo(releaseURL, platform, arch string) (string, string, error) {
	...
	if looksLikeDirectAssetURL(releaseURL) {
		return "", "", fmt.Errorf("для артефакта %s не найден checksum", releaseURL)
	}
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return "", "", fmt.Errorf("не удалось запросить релизы: статус %d", resp.StatusCode)
	}
	...
	if url, idx, ok := pickBest(platformIdx); ok {
		...
		return "", "", fmt.Errorf("для артефакта %s не найден checksum", url)
	}

	// No platform match — require explicit platform+arch; fail fast.
	return "", "", fmt.Errorf("не найден релизный артефакт для platform=%q и arch=%q", platform, arch)
}
...
func (pw *progressWriter) print() {
	if pw.total > 0 {
		...
		fmt.Fprintf(os.Stderr, "\rСкачивание: %s / %s (%.1f%%)", humanBytes(pw.written), humanBytes(pw.total), pct)
	} else {
		fmt.Fprintf(os.Stderr, "\rСкачивание: %s", humanBytes(pw.written))
	}
}
...
func extractZip(archivePath, destDir string) error {
	...
	for _, f := range r.File {
		...
		if !strings.HasPrefix(target, destClean+string(os.PathSeparator)) && target != destClean {
			return fmt.Errorf("обнаружен path traversal: %s", f.Name)
		}
		if f.FileInfo().IsDir() {
...
func extractTarFromReader(tr *tar.Reader, destDir string) error {
	for {
		...
		if !strings.HasPrefix(target, filepath.Clean(destDir)+string(os.PathSeparator)) &&
			...
			return fmt.Errorf("обнаружен path traversal: %s", hdr.Name)
		}
		switch hdr.Typeflag {
		case tar.TypeDir:
...
func findBinaryInDir(dir, programName string) (string, error) {
	baseNames := binaryNameCandidates(programName)
	if len(baseNames) == 0 {
		return "", fmt.Errorf("имя бинарника пустое")
	}

	var wanted []string
	for _, baseName := range baseNames {
		if runtime.GOOS == "windows" {
			wanted = appendUniqueStrings(wanted, baseName+".exe", baseName)
		} else {
			// also accept programs with .exe in archives targeting windows
			wanted = appendUniqueStrings(wanted, baseName, baseName+".exe")
		}
	}
	...
	if found == "" {
		return "", fmt.Errorf("бинарник %q не найден в архиве", programName)
	}
	return found, nil
}

func binaryNameCandidates(programName string) []string {
	name := strings.TrimSpace(strings.TrimSuffix(programName, ".exe"))
	switch name {
	case "":
		return nil
	case "ruclaw", "picoclaw":
		return []string{"ruclaw", "picoclaw"}
	case "ruclaw-launcher", "picoclaw-launcher":
		return []string{"ruclaw-launcher", "picoclaw-launcher"}
	default:
		return []string{name}
	}
}
...
func NewUpdateCommand(binaryName string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Проверить и установить обновления из релизов GitHub",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Println("Обновление установлено. Перезапустите RuClaw, чтобы использовать новую версию.")
			return nil
		},
	}
	cmd.Flags().StringVarP(&urlStr, "url", "u", "", "Прямая ссылка на архив релиза или страницу релиза")
	cmd.Flags().StringVar(&platform, "platform", "", "Целевая платформа (по умолчанию: runtime.GOOS)")
	cmd.Flags().StringVar(&arch, "arch", "", "Целевая архитектура (по умолчанию: runtime.GOARCH)")
	return cmd
}
