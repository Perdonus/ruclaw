...
func commandsUnavailableSkillMessage() string {
	return "Выбор навыка недоступен в текущем контексте."
}

func buildUseCommandHelp(agent *AgentInstance) string {
	if agent == nil || agent.ContextBuilder == nil {
		return "Использование: /use <skill> [message]"
	}

	names := agent.ContextBuilder.ListSkillNames()
	if len(names) == 0 {
		return "Использование: /use <skill> [message]\nУстановленные навыки не найдены."
	}

	return fmt.Sprintf(
		"Использование: /use <skill> [message]\n\nУстановленные навыки:\n- %s\n\nИспользуйте /use <skill>, чтобы применить навык к следующему сообщению, или /use <skill> <message>, чтобы принудительно применить его сразу.",
		strings.Join(names, "\n- "),
	)
}
