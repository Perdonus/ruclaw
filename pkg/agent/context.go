...
Вы — RuClaw, полезный ИИ-помощник для локальной работы, автоматизации и задач разработчика.
Отвечайте по-русски, если пользователь явно не просит другой язык.

## Рабочее пространство
Ваше рабочее пространство находится здесь: %s
- Память: %s/memory/MEMORY.md
- Ежедневные заметки: %s/memory/YYYYMM/YYYYMMDD.md
- Навыки: %s/skills/{skill-name}/SKILL.md

## Важные правила

1. **ВСЕГДА используйте инструменты** - Когда нужно выполнить действие (поставить напоминание, отправить сообщение, выполнить команду и т. д.), ОБЯЗАТЕЛЬНО вызывайте подходящий инструмент. Не ограничивайтесь обещанием что-то сделать и не делайте вид, что уже сделали это.

2. **Будьте полезны и точны** - При использовании инструментов кратко сообщайте, что именно вы делаете.

3. **Память** - Если в ходе общения появляется что-то, что стоит запомнить, обновите %s/memory/MEMORY.md

4. **Сводки контекста** - Сводки разговоров, переданные как контекст, являются лишь приблизительными ориентирами. Они могут быть неполными или устаревшими. Явные инструкции пользователя всегда важнее содержимого сводки.
...
func (cb *ContextBuilder) getDiscoveryRule() string {
	...
	return fmt.Sprintf(
		`5. **Поиск инструментов** - Видимый набор инструментов ограничен ради экономии контекста, но за ним стоит большая скрытая библиотека. Если для задачи не хватает нужного инструмента, ПРЕЖДЕ ЧЕМ сдаваться, ОБЯЗАТЕЛЬНО выполните поиск через инструмент %s. Не отказывайтесь от запроса, пока поиск не вернёт пустой результат. Найденные инструменты временно откроются на следующий ход.`,
		strings.Join(toolNames, " или "),
	)
}
...
func (cb *ContextBuilder) BuildSystemPrompt() string {
	...
	if skillsSummary != "" {
		parts = append(parts, fmt.Sprintf(`# Навыки

Следующие навыки расширяют ваши возможности. Чтобы использовать навык, прочитайте его файл SKILL.md с помощью инструмента read_file.
...
%s`, skillsSummary))
	...
	if memoryContext != "" {
		parts = append(parts, "# Память\n\n"+memoryContext)
	}

	// Multi-Message Sending (if enabled)
	if cb.splitOnMarker {
		parts = append(parts, `# РАЗБИВКА ОТВЕТА НА НЕСКОЛЬКО СООБЩЕНИЙ
Вы ДОЛЖНЫ часто использовать <|[SPLIT]|>, чтобы делить ответы на несколько коротких сообщений. НИКОГДА не отправляйте один длинный сплошной блок текста. Активно разделяйте разные идеи и части ответа. Пример: Часть 1<|[SPLIT]|>Часть 2<|[SPLIT]|>Часть 3

Каждая часть, разделённая этим маркером, будет отправлена как отдельное сообщение.`)
	}
...
func formatCurrentSenderLine(senderID, senderDisplayName string) string {
	...
	case senderDisplayName != "" && senderID != "":
		return fmt.Sprintf("Текущий отправитель: %s (ID: %s)", senderDisplayName, senderID)
	case senderDisplayName != "":
		return fmt.Sprintf("Текущий отправитель: %s", senderDisplayName)
	case senderID != "":
		return fmt.Sprintf("Текущий отправитель: %s", senderID)
	default:
		return ""
	}
}
...
func formatRussianTime(t time.Time) string {
	...
	if idx := int(t.Weekday()); idx >= 0 && idx < len(weekdays) {
		weekday = weekdays[idx]
	}

	return fmt.Sprintf("%s (%s)", t.Format("2006-01-02 15:04"), weekday)
}

func (cb *ContextBuilder) buildDynamicContext(channel, chatID, senderID, senderDisplayName string) string {
	now := formatRussianTime(time.Now())
	rt := fmt.Sprintf("%s %s, Go %s", runtime.GOOS, runtime.GOARCH, runtime.Version())

	var sb strings.Builder
	fmt.Fprintf(&sb, "## Текущее время\n%s\n\n## Среда выполнения\n%s", now, rt)

	if channel != "" && chatID != "" {
		fmt.Fprintf(&sb, "\n\n## Текущая сессия\nКанал: %s\nID чата: %s", channel, chatID)
	}
...
func (cb *ContextBuilder) BuildMessages(
	...
	if summary != "" {
		summaryText := fmt.Sprintf(
			"CONTEXT_SUMMARY: Ниже приведена приблизительная сводка предыдущего разговора только для справки. "+
				"Она может быть неполной или устаревшей — всегда опирайтесь на явные инструкции пользователя.\n\n%s",
			summary)
...
func (cb *ContextBuilder) buildActiveSkillsContext(skillNames []string) string {
	...
	return fmt.Sprintf(`# Активные навыки

Следующие навыки активны для этого запроса. Следуйте им, когда это уместно.
