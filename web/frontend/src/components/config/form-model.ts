...
export function parseIntField(
  ...
  if (options.min !== undefined && value < options.min) {
    throw new Error(`Поле «${label}» должно быть не меньше ${options.min}.`)
  }
  if (options.max !== undefined && value > options.max) {
    throw new Error(`Поле «${label}» должно быть не больше ${options.max}.`)
  }
  return value
}
