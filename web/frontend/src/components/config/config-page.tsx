  ...
  const handleSave = async () => {
    try {
      ...
      if (configDirty) {
        ...
        if (!workspace) {
          throw new Error("Нужно указать директорию workspace.")
        }
        if (!dmScope) {
          throw new Error("Нужно указать область сессии.")
        }

        const maxTokens = parseIntField(form.maxTokens, "Макс. токены", {
          min: 1,
        })
        const contextWindow = form.contextWindow.trim()
          ? parseIntField(form.contextWindow, "Контекстное окно", { min: 1 })
          : undefined
        const maxToolIterations = parseIntField(
          form.maxToolIterations,
          "Макс. итерации инструментов",
          { min: 1 },
        )
        const toolFeedbackMaxArgsLength = parseIntField(
          form.toolFeedbackMaxArgsLength,
          "Длина предпросмотра аргументов инструмента",
          { min: 0 },
        )
        const summarizeMessageThreshold = parseIntField(
          form.summarizeMessageThreshold,
          "Порог суммаризации сообщений",
          { min: 1 },
        )
        const summarizeTokenPercent = parseIntField(
          form.summarizeTokenPercent,
          "Процент токенов для суммаризации",
          { min: 1, max: 100 },
        )
      ...
      if (launcherDirty) {
        const port = parseIntField(launcherForm.port, "Порт сервиса", {
          min: 1,
          max: 65535,
        })
