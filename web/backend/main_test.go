...
func TestLanguageFromArgs(t *testing.T) {
	tests := []struct {
		name string
		args []string
		want string
	}{
		{name: "short flag separate arg", args: []string{"-lang", "en"}, want: "en"},
		{name: "short flag inline", args: []string{"-lang=zh"}, want: "zh"},
		{name: "long flag separate arg", args: []string{"--lang", "ru"}, want: "ru"},
		{name: "long flag inline", args: []string{"--lang=en"}, want: "en"},
		{name: "missing flag", args: []string{"-public"}, want: ""},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := languageFromArgs(tt.args); got != tt.want {
				t.Fatalf("languageFromArgs(%v) = %q, want %q", tt.args, got, tt.want)
			}
		})
	}
}
