...
func (h *Handler) handleStartWeixinFlow(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, TranslateLauncher(WeixinCreateClientFailed, err), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) handlePollWeixinFlow(w http.ResponseWriter, r *http.Request) {
	...
	api, err := weixin.NewApiClient(weixinBaseURL, "", "")
	if err != nil {
		h.setWeixinFlowError(flowID, TranslateLauncher(WeixinClientFailed, err))
	...
	case weixinStatusConfirmed:
		if statusResp.BotToken == "" {
			h.setWeixinFlowError(flowID, TranslateLauncher(WeixinMissingBotToken))
			break
		}
		if saveErr := h.saveWeixinBinding(statusResp.BotToken, statusResp.IlinkBotID); saveErr != nil {
			h.setWeixinFlowError(flowID, TranslateLauncher(WeixinSaveTokenFailed, saveErr))
