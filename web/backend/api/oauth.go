...
func (h *Handler) handleListOAuthProviders(w http.ResponseWriter, r *http.Request) {
	...
	for _, provider := range oauthProviderOrder {
		...
		if err != nil {
			http.Error(w, TranslateLauncher(OAuthLoadCredentialsFailed, err), http.StatusInternalServerError)
			return
		}
...
func (h *Handler) handleOAuthLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, TranslateLauncher(OAuthReadRequestFailed), http.StatusBadRequest)
		return
	}
	defer r.Body.Close()
	...
	if err = json.Unmarshal(body, &req); err != nil {
		http.Error(w, TranslateLauncher(OAuthInvalidJSON, err), http.StatusBadRequest)
		return
	}
	...
	if !isOAuthMethodSupported(provider, method) {
		http.Error(
			w,
			TranslateLauncher(OAuthUnsupportedMethodForProvider, method, provider),
	...
	case oauthMethodToken:
		...
		if token == "" {
			http.Error(w, TranslateLauncher(OAuthTokenRequired), http.StatusBadRequest)
			return
		}
		...
		if err := h.persistCredentialAndConfig(provider, oauthMethodToken, cred); err != nil {
			http.Error(w, TranslateLauncher(OAuthTokenLoginFailed, err), http.StatusInternalServerError)
			return
		}
	...
	case oauthMethodDeviceCode:
		...
		if err != nil {
			http.Error(w, TranslateLauncher(OAuthDeviceCodeRequestFailed, err), http.StatusInternalServerError)
			return
		}
	...
	case oauthMethodBrowser:
		...
		if err != nil {
			http.Error(w, TranslateLauncher(OAuthGeneratePKCEFailed, err), http.StatusInternalServerError)
			return
		}
		state, err := oauthGenerateState()
		if err != nil {
			http.Error(w, TranslateLauncher(OAuthGenerateStateFailed, err), http.StatusInternalServerError)
			return
		}
		...
		return
	default:
		http.Error(w, TranslateLauncher(OAuthUnsupportedMethod), http.StatusBadRequest)
	}
}
...
func (h *Handler) handleGetOAuthFlow(w http.ResponseWriter, r *http.Request) {
	...
	if flowID == "" {
		http.Error(w, TranslateLauncher(OAuthMissingFlowID), http.StatusBadRequest)
		return
	}

	flow, ok := h.getOAuthFlow(flowID)
	if !ok {
		http.Error(w, TranslateLauncher(OAuthFlowNotFound), http.StatusNotFound)
		return
	}
...
func (h *Handler) handlePollOAuthFlow(w http.ResponseWriter, r *http.Request) {
	...
	if flow.Method != oauthMethodDeviceCode {
		http.Error(w, TranslateLauncher(OAuthFlowNoPolling), http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		...
		h.setOAuthFlowError(flowID, TranslateLauncher(OAuthDeviceCodePollFailed, err))
		updated, _ := h.getOAuthFlow(flowID)
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(flowToResponse(updated))
		return
	...
	if err := h.persistCredentialAndConfig(flow.Provider, oauthMethodTokenOrOAuth(flow.Method), cred); err != nil {
		h.setOAuthFlowError(flowID, TranslateLauncher(OAuthSaveCredentialFailed, err))
		updated, _ := h.getOAuthFlow(flowID)
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(flowToResponse(updated))
		return
...
func (h *Handler) handleOAuthCallback(w http.ResponseWriter, r *http.Request) {
	...
	if state == "" {
		renderOAuthCallbackPage(w, "", oauthFlowError, TranslateLauncher(OAuthCallbackMissingStateTitle), "missing_state")
		return
	}

	flow, ok := h.getOAuthFlowByState(state)
	if !ok {
		renderOAuthCallbackPage(w, "", oauthFlowError, TranslateLauncher(OAuthCallbackFlowNotFoundTitle), "flow_not_found")
		return
	}

	if flow.Status != oauthFlowPending {
		renderOAuthCallbackPage(w, flow.ID, flow.Status, TranslateLauncher(OAuthCallbackAlreadyCompletedTitle), flow.Error)
		return
	}
	...
	if errMsg := strings.TrimSpace(r.URL.Query().Get("error")); errMsg != "" {
		...
		renderOAuthCallbackPage(w, flow.ID, oauthFlowError, TranslateLauncher(OAuthCallbackAuthorizationFailedTitle), errMsg)
		return
	}

	code := strings.TrimSpace(r.URL.Query().Get("code"))
	if code == "" {
		h.setOAuthFlowError(flow.ID, TranslateLauncher(OAuthCallbackMissingCodeTitle))
		renderOAuthCallbackPage(w, flow.ID, oauthFlowError, TranslateLauncher(OAuthCallbackMissingCodeTitle), "missing_code")
		return
	}
	...
	if err != nil {
		...
		renderOAuthCallbackPage(w, flow.ID, oauthFlowError, TranslateLauncher(OAuthCallbackUnsupportedProviderTitle), err.Error())
		return
	}

	cred, err := oauthExchangeCodeForTokens(cfg, code, flow.CodeVerifier, flow.RedirectURI)
	if err != nil {
		h.setOAuthFlowError(flow.ID, TranslateLauncher(OAuthCallbackTokenExchangeFailedTitle)+": "+err.Error())
		renderOAuthCallbackPage(w, flow.ID, oauthFlowError, TranslateLauncher(OAuthCallbackTokenExchangeFailedTitle), err.Error())
		return
	}

	if err := h.persistCredentialAndConfig(flow.Provider, oauthMethodTokenOrOAuth(flow.Method), cred); err != nil {
		h.setOAuthFlowError(flow.ID, TranslateLauncher(OAuthSaveCredentialFailed, err))
		renderOAuthCallbackPage(w, flow.ID, oauthFlowError, TranslateLauncher(OAuthCallbackSaveCredentialFailedTitle), err.Error())
		return
	}

	h.setOAuthFlowSuccess(flow.ID)
	renderOAuthCallbackPage(w, flow.ID, oauthFlowSuccess, TranslateLauncher(OAuthCallbackSuccessTitle), "")
}
...
func (h *Handler) handleOAuthLogout(w http.ResponseWriter, r *http.Request) {
	...
	if err := oauthDeleteCredential(provider); err != nil {
		http.Error(w, TranslateLauncher(OAuthDeleteCredentialFailed, err), http.StatusInternalServerError)
		return
	}
	if err := h.syncProviderAuthMethod(provider, ""); err != nil {
		http.Error(w, TranslateLauncher(OAuthUpdateConfigFailed, err), http.StatusInternalServerError)
		return
	}
...
func renderOAuthCallbackPage(w http.ResponseWriter, flowID, status, title, errMsg string) {
	...
	_, _ = fmt.Fprintf(
		w,
		"<!doctype html><html><head><meta charset=\"utf-8\"><title>%s</title></head><body><script>(function(){var payload=%s;var hasOpener=false;try{if(window.opener&&!window.opener.closed){window.opener.postMessage(payload,window.location.origin);hasOpener=true}}catch(e){}var target='/credentials?oauth_flow_id='+encodeURIComponent(payload.flowId||'')+'&oauth_status='+encodeURIComponent(payload.status||'');setTimeout(function(){if(hasOpener){window.close();return}window.location.replace(target)},800)})();</script><div style=\"font-family:Inter,system-ui,sans-serif;padding:24px\"><h2>%s</h2><p>%s</p><p>%s</p></div></body></html>",
		html.EscapeString(TranslateLauncher(OAuthCallbackWindowTitle)),
		string(payloadJSON),
		html.EscapeString(title),
		html.EscapeString(message),
		html.EscapeString(TranslateLauncher(OAuthCallbackCloseWindow)),
	)
}
...
func normalizeOAuthProvider(raw string) (string, error) {
	...
	case oauthProviderOpenAI, oauthProviderAnthropic, oauthProviderGoogleAntigravity:
		return provider, nil
	default:
		return "", fmt.Errorf(TranslateLauncher(OAuthUnsupportedProvider, raw))
	}
}
...
func oauthConfigForProvider(provider string) (auth.OAuthProviderConfig, error) {
	...
	default:
		return auth.OAuthProviderConfig{}, fmt.Errorf(TranslateLauncher(OAuthProviderNoBrowser, provider))
	}
}
...
func (h *Handler) gcOAuthFlowsLocked(now time.Time) {
	for id, flow := range h.oauthFlows {
		if flow.Status == oauthFlowPending && !flow.ExpiresAt.IsZero() && now.After(flow.ExpiresAt) {
			...
			flow.Error = TranslateLauncher(OAuthFlowExpired)
			flow.UpdatedAt = now
			if flow.OAuthState != "" {
				delete(h.oauthState, flow.OAuthState)
			}
...
func (h *Handler) persistCredentialAndConfig(provider, authMethod string, cred *auth.AuthCredential) error {
	if cred == nil {
		return fmt.Errorf(TranslateLauncher(OAuthEmptyCredential))
	}
	...
	if err := oauthSetCredential(provider, &cp); err != nil {
		return fmt.Errorf(TranslateLauncher(OAuthPersistCredentialFailed), err)
	}
	if err := h.syncProviderAuthMethod(provider, authMethod); err != nil {
		return fmt.Errorf(TranslateLauncher(OAuthSyncProviderConfigFailed), err)
	}
	return nil
}
