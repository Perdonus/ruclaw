...
import (
	...
	ppid "github.com/Perdonus/ruclaw/pkg/pid"
)
...
func (h *Handler) createWsProxy(origProtocol string, token string) *httputil.ReverseProxy {
	wsProxy := &httputil.ReverseProxy{
		...
		ErrorHandler: func(w http.ResponseWriter, r *http.Request, err error) {
			logger.Errorf(TranslateLauncher(PicoProxyFailed, err))
			http.Error(w, TranslateLauncher(PicoProxyFailed, err), http.StatusBadGateway)
		},
	}
	return wsProxy
}
...
func (h *Handler) handleWebSocketProxy() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !gatewayAvailable {
			logger.Warnf(TranslateLauncher(PicoGatewayUnavailable))
			http.Error(w, TranslateLauncher(PicoGatewayNotAvailable), http.StatusServiceUnavailable)
			return
		}
		...
		logger.Warnf("%s: %v", TranslateLauncher(PicoInvalidToken), prot)
		http.Error(w, TranslateLauncher(PicoInvalidToken), http.StatusForbidden)
	}
}
...
func (h *Handler) handleGetPicoToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, TranslateLauncher(PicoConfigLoadFailed, err), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) handleRegenPicoToken(w http.ResponseWriter, r *http.Request) {
	...
	if err := config.SaveConfig(h.configPath, cfg); err != nil {
		http.Error(w, TranslateLauncher(PicoConfigSaveFailed, err), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) EnsurePicoChannel(callerOrigin string) (bool, error) {
	cfg, err := config.LoadConfig(h.configPath)
	if err != nil {
		return false, fmt.Errorf(TranslateLauncher(PicoEnsureConfigLoadFailed), err)
	}
	...
	if changed {
		if err := config.SaveConfig(h.configPath, cfg); err != nil {
			return false, fmt.Errorf(TranslateLauncher(PicoEnsureConfigSaveFailed), err)
		}
	}
