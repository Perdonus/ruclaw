...
func (h *Handler) handleStartWecomFlow(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, TranslateLauncher(QRCodeFetchFailed, err), http.StatusInternalServerError)
		return
	}

	dataURI, err := generateQRDataURI(session.Data.AuthURL)
	if err != nil {
		http.Error(w, TranslateLauncher(QRImageGenerateFailed, err), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) handlePollWecomFlow(w http.ResponseWriter, r *http.Request) {
	...
	if flowID == "" {
		http.Error(w, TranslateLauncher(FlowMissingID), http.StatusBadRequest)
		return
	}

	flow, ok := h.getWecomFlow(flowID)
	if !ok {
		http.Error(w, TranslateLauncher(FlowNotFound), http.StatusNotFound)
		return
	}
	...
	case "success":
		if statusResp.Data.BotInfo.BotID == "" || statusResp.Data.BotInfo.Secret == "" {
			h.setWecomFlowError(flowID, TranslateLauncher(WecomMissingBotCredentials))
			break
		}
		if saveErr := h.saveWecomBinding(
			...
			h.setWecomFlowError(flowID, TranslateLauncher(WecomSaveCredentialsFailed, saveErr))
...
func fetchWecomQRCode(ctx context.Context) (wecomQRGenerateResponse, error) {
	...
	if resp.Data.SCode == "" || resp.Data.AuthURL == "" {
		return wecomQRGenerateResponse{}, fmt.Errorf(TranslateLauncher(WecomMissingResponseFields))
	}
	return resp, nil
}
...
func buildWecomQRGenerateURL(baseURL, sourceID string, platformCode int) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf(TranslateLauncher(WecomInvalidGenerateURL), err)
	}
...
func buildWecomQRQueryURL(baseURL, scode string) (string, error) {
	u, err := url.Parse(baseURL)
	if err != nil {
		return "", fmt.Errorf(TranslateLauncher(WecomInvalidQueryURL), err)
	}
