...
func (h *Handler) handleGetLauncherConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, TranslateLauncher(LauncherConfigReadFailed, err), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) handleUpdateLauncherConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
		http.Error(w, TranslateLauncher(LauncherConfigInvalidJSON, err), http.StatusBadRequest)
		return
	}
	...
	if err := launcherconfig.Save(h.launcherConfigPath(), cfg); err != nil {
		http.Error(w, TranslateLauncher(LauncherConfigSaveFailed, err), http.StatusInternalServerError)
		return
	}
